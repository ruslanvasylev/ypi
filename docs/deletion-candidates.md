...
| Preserved incumbent native engine in `extensions/ypi/legacy-native-tool.ts` | **marked for deletion; retained and tested** | `extensions/ypi/runtime-core.ts` plus thin `extensions/ypi/native-tool.ts` Pi adapter | 75-case shared contract; 111-case native harness; bounded/cancellation/cost tests; 22-case pi-recursive pack; canonical and retained E9 both prove a native root-to-child transition | `YPI_LEGACY_IMPL=1` | removal not approved; separate maintainer decision required after ≥1 release |
