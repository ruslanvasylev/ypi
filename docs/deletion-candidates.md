...
|---|---|---|---|---|---|
| Preserved incumbent CLI engine in `rlm_query.legacy` | **marked for deletion; retained and tested** | `extensions/ypi/runtime-core.ts` plus thin `extensions/ypi/cli.ts` and `rlm_query` launcher | 75-case shared contract; 176-case guardrails; byte-preserving stdin/stream/async/cancellation tests; 37-case packed consumer; exact long-context real-model answer with observed depth-1 and depth-2 transitions; final live E1–E9 gate 10/10 | `YPI_LEGACY_IMPL=1` | removal not approved; separate maintainer decision required after ≥1 release |
| Preserved incumbent native engine in `extensions/ypi/legacy-native-tool.ts` | **marked for deletion; retained and tested** | `extensions/ypi/runtime-core.ts` plus thin `extensions/ypi/native-tool.ts` Pi adapter | 75-case shared contract; 75-case native harness; bounded/cancellation/cost tests; 19-case pure-extension pack; canonical and retained E9 both prove a native root-to-child transition | `YPI_LEGACY_IMPL=1` | removal not approved; separate maintainer decision required after ≥1 release |
