...
import { chmodSync, copyFileSync, existsSync as existsSync3, mkdtempSync, readFileSync, readdirSync, rmSync, writeFileSync } from "node:fs";
...
function createAsyncJob(input) {
  ...
  try {
    ...
    } else if (input.contextPath && existsSync3(input.contextPath)) {
      ownedContextPath = snapshotFile(input.contextPath, path3.join(jobDir, "context.txt"));
    }
    let parentSessionSnapshot;
    if (input.fork && process.env.RLM_SESSION_FILE && existsSync3(process.env.RLM_SESSION_FILE)) {
...
function discardAsyncJob(job, workerPid = 0) {
  if (workerPid > 0 && !existsSync3(job.admissionPath) && !existsSync3(job.sentinelPath)) {
...
async function waitForAsyncAdmission(job, timeoutMilliseconds = 30000) {
  ...
  while (Date.now() < deadline) {
    if (existsSync3(job.admissionPath))
      return;
    if (existsSync3(job.sentinelPath)) {
...
import { closeSync, existsSync as existsSync4, mkdtempSync as mkdtempSync2, openSync, rmSync as rmSync2, writeSync } from "node:fs";
...
import { constants, existsSync as existsSync5, mkdirSync as mkdirSync2, readFileSync as readFileSync2, rmSync as rmSync3, writeFileSync as writeFileSync2 } from "node:fs";
...
function readCounter(filePath) {
  const raw = existsSync5(filePath) ? readFileSync2(filePath, "utf8").trim() || "0" : process.env.RLM_CALL_COUNT || "0";
...
function readCostSummary(costFile = process.env.RLM_COST_FILE) {
  if (!costFile || !existsSync5(costFile)) {
