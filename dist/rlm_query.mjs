...
import { accessSync, constants as constants4, existsSync as existsSync3, mkdirSync as mkdirSync2, realpathSync as realpathSync3 } from "node:fs";
...
function canonicalPrivateFilePath(candidate) {
  if (!path2.isAbsolute(candidate) || path2.normalize(candidate) !== candidate) {
    throw controlError("Private file path must be absolute and normalized");
  }
  const parent = realpathSync.native(path2.dirname(candidate));
  return path2.join(parent, path2.basename(candidate));
}
...
// extensions/ypi/internal/root-session.ts
import {
...
var activeRootBinding;
function controlError3(message) {
  return new Error(message);
}
function assertOwner(metadata, candidate) {
  const uid = process.getuid?.();
  if (uid !== undefined && metadata.uid !== BigInt(uid)) {
    throw controlError3(`Root session path is owned by another user: ${candidate}`);
  }
}
...
function identityOf3(metadata) {
  return {
    ...
    inode: metadata.ino.toString(),
    kind: "file",
    mode: process.platform === "win32" ? PRIVATE_FILE_MODE : Number(metadata.mode & 0o777n),
    links: metadata.nlink.toString()
  };
}
function canonicalRootSessionFilePath(candidate) {
  try {
    ...
    if (error instanceof Error && error.message === "Private file path must be absolute and normalized") {
      throw controlError3("Root session path must be absolute and normalized");
    }
    throw error;
  }
}
function hardenActiveRootSessionFile(candidate) {
  ...
  if (activeRootBinding?.path !== canonical)
    activeRootBinding = undefined;
  const canonicalParent = path4.dirname(canonical);
  const parentMetadata = lstatSync3(canonicalParent, { bigint: true });
  if (!parentMetadata.isDirectory() || parentMetadata.isSymbolicLink()) {
    throw controlError3(`Root session parent is not a regular directory: ${canonicalParent}`);
  }
  assertOwner(parentMetadata, canonicalParent);
  let namedBefore;
  try {
    namedBefore = lstatSync3(canonical, { bigint: true });
  } catch (error) {
    if (error.code === "ENOENT") {
      delete process.env.YPI_ROOT_SESSION_FILE_IDENTITY;
      return;
  ...
  if (!namedBefore.isFile() || namedBefore.isSymbolicLink()) {
    throw controlError3(`Root session path is not a regular file: ${canonical}`);
  }
  assertOwner(namedBefore, canonical);
  if (namedBefore.nlink !== 1n) {
    throw controlError3(`Root session file must be singly linked: ${canonical}`);
  }
  const previous = activeRootBinding?.identity ?? (process.env.YPI_ROOT_SESSION_FILE_IDENTITY ? parsePrivateFileIdentity(process.env.YPI_ROOT_SESSION_FILE_IDENTITY) : undefined);
  if (previous) {
    if (previous.device !== namedBefore.dev.toString() || previous.inode !== namedBefore.ino.toString()) {
      throw controlError3(`Root session file identity changed: ${canonical}`);
    }
  }
  const descriptor = openSync3(canonical, constants3.O_RDONLY | (constants3.O_NOFOLLOW || 0));
  try {
    const opened = fstatSync3(descriptor, { bigint: true });
    if (!opened.isFile() || !sameInode(namedBefore, opened)) {
      throw controlError3(`Root session identity changed before hardening: ${canonical}`);
    }
    ...
    const hardened = fstatSync3(descriptor, { bigint: true });
    const namedAfter = lstatSync3(canonical, { bigint: true });
    const parentAfter = lstatSync3(canonicalParent, { bigint: true });
    if (!sameInode(opened, hardened) || !sameInode(opened, namedAfter) || namedAfter.isSymbolicLink() || !sameInode(parentMetadata, parentAfter) || hardened.nlink !== 1n || process.platform !== "win32" && Number(hardened.mode & 0o777n) !== PRIVATE_FILE_MODE) {
      throw controlError3(`Root session identity changed during hardening: ${canonical}`);
    }
    const identity = identityOf3(hardened);
...
function normalizedPath(filePath) {
  ...
  return existsSync2(resolved) ? realpathSync2(resolved) : resolved;
}
function resolveRuntime(importMetaUrl) {
  const extensionPath = fileURLToPath(importMetaUrl);
  const extensionDir = path5.dirname(extensionPath);
  const defaultRoot = path5.resolve(extensionDir, "..");
  ...
  const root = path5.resolve(configuredRoot && configuredExtensionMatches ? configuredRoot : defaultRoot);
  return {
    extensionPath,
    extensionDir,
    root,
    systemPromptPath: path5.join(root, "SYSTEM_PROMPT.md"),
    rlmQueryPath: path5.join(root, "rlm_query"),
    runtimeCorePath: path5.join(root, "extensions", "ypi", "runtime-core.ts"),
    runtimeInternalDir: path5.join(root, "extensions", "ypi", "internal"),
    cliAdapterPath: path5.join(root, "extensions", "ypi", "cli.ts")
  };
}
...
var rootSessionWarning;
var lastNotifiedRootSessionWarning;
function boundedFailure(error) {
  const message = error instanceof Error ? error.message : String(error);
  return message.replace(/[\r\n\t]+/g, " ").slice(0, 240);
}
function clearRootSessionProjection() {
  delete process.env.RLM_SESSION_FILE;
  delete process.env.RLM_SESSION_DIR;
  delete process.env.YPI_ROOT_SESSION_FILE_IDENTITY;
}
function recordRootSessionFailure(error, ctx) {
...
function clearRootSessionFailure() {
...
function prependPath(dir) {
  ...
  const entries = current.split(path6.delimiter).filter(Boolean);
  if (!entries.includes(dir)) {
    process.env.PATH = [dir, ...entries].join(path6.delimiter);
  }
}
...
function ensurePiExecutable(runtime) {
  if (process.env.YPI_PI_BIN)
    return;
  const candidate = path6.join(runtime.root, "node_modules", ".bin", "pi");
  try {
...
function ensureRuntimeStatePaths() {
  ...
  const stateRoot = realpathSync3.native(createPrivateTempDirectory(path6.join(tmpdir(), `ypi_runtime_${stateLabel}_`)));
  process.env.RLM_CALL_COUNTER_FILE ||= path6.join(stateRoot, "calls.counter");
  process.env.RLM_CONCURRENCY_DIR ||= path6.join(stateRoot, "concurrency");
  process.env.PI_TRACE_FILE ||= path6.join(stateRoot, "trace.jsonl");
  process.env.RLM_COST_FILE ||= path6.join(stateRoot, "cost.jsonl");
}
  ...
  if (ctx && sharedSessionsEnabled()) {
    ...
    if (sessionFile) {
      try {
        ...
        const canonicalDirectory = realpathSync3.native(ctx.sessionManager.getSessionDir());
        if (path6.dirname(canonicalFile) !== canonicalDirectory) {
...
async function allocateCallCount(deadlineMilliseconds, signal) {
  ...
  const counterFile = process.env.RLM_CALL_COUNTER_FILE || path7.join(tmpdir2(), "rlm_calls_default.counter");
...
function assertInsideJobDir(job, candidate) {
  const jobDir = path8.dirname(path8.resolve(job.jobPath));
  if (path8.dirname(path8.resolve(candidate)) !== jobDir) {
...
function createAsyncJob(input) {
  ...
  const jobDir = createPrivateTempDirectory(path8.join(root, `rlm_async_${traceId}_`));
  const jobPath = path8.join(jobDir, "job.json");
  const outputPath = path8.join(jobDir, "output.txt");
  const sentinelPath = path8.join(jobDir, "done");
  const admissionPath = path8.join(jobDir, "admitted");
  const childPidPath = path8.join(jobDir, "child.pid");
  try {
    atomicCreateFile(outputPath, "");
    let ownedContextPath;
    if (input.context !== undefined) {
      ownedContextPath = path8.join(jobDir, "context.txt");
      atomicCreateFile(ownedContextPath, input.context);
    } else if (input.contextPath && existsSync4(input.contextPath)) {
      ownedContextPath = snapshotFile(input.contextPath, path8.join(jobDir, "context.txt"));
    }
    let parentSessionSnapshot;
    if (input.fork && process.env.RLM_SESSION_FILE && existsSync4(process.env.RLM_SESSION_FILE)) {
      parentSessionSnapshot = snapshotFile(process.env.RLM_SESSION_FILE, path8.join(jobDir, "parent-session.jsonl"));
    }
    let rootPromptSnapshot;
    if (process.env.RLM_ROOT_PROMPT_FILE && existsSync4(process.env.RLM_ROOT_PROMPT_FILE)) {
      rootPromptSnapshot = snapshotFile(process.env.RLM_ROOT_PROMPT_FILE, path8.join(jobDir, "root-prompt.txt"));
    }
    return {
      prompt: input.prompt,
      fork: input.fork,
...
function readAsyncJob(jobPath) {
  ...
  if (path8.resolve(job.jobPath) !== path8.resolve(jobPath))
...
function discardAsyncJob(job, workerPid = 0) {
  if (workerPid > 0 && !existsSync4(job.sentinelPath)) {
    throw new Error(`Refusing to discard non-terminal async job state: ${path8.dirname(job.jobPath)}`);
  }
  rmSync(path8.dirname(job.jobPath), { recursive: true, force: true });
}
...
import { closeSync as closeSync4, existsSync as existsSync5, fchmodSync as fchmodSync4, openSync as openSync4, writeSync as writeSync2 } from "node:fs";
...
async function spoolStdin(options) {
  const owner = createOwnedPrivateTempDirectory(path9.join(process.env.TMPDIR || tmpdir4(), "ypi_cli_stdin_"));
  const contextPath = path9.join(owner.path, "context.bin");
  let descriptor;
  ...
  try {
    descriptor = openSync4(contextPath, "wx", PRIVATE_FILE_MODE);
    contextCreated = true;
    fchmodSync4(descriptor, PRIVATE_FILE_MODE);
    if (options.signal?.aborted)
...
function removePathEntry(currentPath, entry) {
  if (!currentPath)
    return currentPath;
  return currentPath.split(path10.delimiter).filter((candidate) => candidate && candidate !== entry).join(path10.delimiter);
}
...
function assertOwner2(owner, label) {
...
function assertPrivateRegularFile(metadata, label) {
  ...
  assertOwner2(metadata.uid, label);
...
function openSecureDirectory(directoryPath) {
  if (!path11.isAbsolute(directoryPath)) {
    throw proofError(`RLM_REQUIRE_TRANSCRIPTS=1 requires an absolute session directory: ${directoryPath}`);
  }
  const normalized = path11.resolve(directoryPath);
  let resolved;
  try {
    resolved = realpathSync4.native(normalized);
  } catch {
  ...
  try {
    ...
    assertOwner2(metadata.uid, "Required transcript session directory");
...
function assertDirectoryIdentity(directory) {
  ...
  assertOwner2(descriptorMetadata.uid, "Required transcript session directory");
...
function openSecureForkSource(sourcePath) {
  if (!path12.isAbsolute(sourcePath)) {
...
function createHeldTranscript(childSession, directory, baselineSource) {
  ...
  if (path12.dirname(childSession) !== expectedParent) {
    throw proofError("Required child transcript must be a direct child of the session directory.");
  }
  const temporary = path12.join(expectedParent, `.${path12.basename(childSession)}.${process.pid}.${randomBytes6(8).toString("hex")}.tmp`);
  let descriptor;
  let temporaryRetired = false;
  try {
    descriptor = openSync6(temporary, constants6.O_CREAT | constants6.O_EXCL | constants6.O_RDWR | (constants6.O_NOFOLLOW || 0), PRIVATE_FILE_MODE2);
    fchmodSync5(descriptor, PRIVATE_FILE_MODE2);
    ...
    const identity = identityOf4(metadata);
    return {
      ...identity,
...
function prepareTranscriptProof(input) {
  ...
  if (!path12.isAbsolute(input.childSession)) {
    throw proofError(`RLM_REQUIRE_TRANSCRIPTS=1 requires an absolute child transcript path: ${input.childSession}`);
  }
  const childSession = path12.resolve(input.childSession);
  const directory = openSecureDirectory(path12.dirname(childSession));
  try {
...
function finalizeTranscriptProof(lease, identity) {
  ...
  const receipt = {
    ...
    transcript_file: path12.basename(lease.childSession),
...
function normalizeRepositoryRelativePath(value, label) {
  ...
  if (path13.posix.isAbsolute(value)) {
    throw new Error(`${label} must use repository-relative paths, not absolute paths`);
  }
  const normalized = path13.posix.normalize(value);
...
function isLeaseResourceInventory(value) {
  ...
  for (const [relativePath, identity] of Object.entries(value)) {
    if (relativePath === "" || relativePath === "." || relativePath === "lease.json" || path14.isAbsolute(relativePath) || path14.normalize(relativePath) !== relativePath || relativePath === ".." || relativePath.startsWith(`..${path14.sep}`) || !isLeaseResourceIdentity(identity)) {
      return false;
    }
  }
  return true;
...
function readLeaseJournal(leaseDirectory, expectedToken, commonGitDir) {
  const recordPath = path15.join(leaseDirectory, "lease.json");
...
function writeImplementerLeaseFile(leaseDirectory, record, options = {}) {
  ...
  appendOwnedPrivateFileTransaction(path15.join(leaseDirectory, "lease.json"), record.leaseFileIdentity, persisted.observedBytes, persisted.committedBytes, payload, commitRecord(next, payload), options);
...
function implementerRegistryPaths(commonGitDir) {
  const root = path16.join(commonGitDir, "ypi-implementers");
  return {
    root,
    leases: path16.join(root, "leases"),
    lock: path16.join(commonGitDir, "ypi-implementers.lock"),
    retired: path16.join(root, "retired"),
    staging: path16.join(root, "staging")
  };
}
...
function createImplementerMutex(lockPath, mutexToken, createdAtEpochSeconds = Math.floor(Date.now() / 1000)) {
  ...
  const ownerPath = path17.join(lockPath, "owner.json");
...
function normalizeRelativePath(relativePath) {
  if (relativePath === "" || relativePath === "." || relativePath === "lease.json" || path18.isAbsolute(relativePath)) {
    throw new Error(`invalid implementer lease resource path: ${relativePath}`);
  }
  const normalized = path18.normalize(relativePath);
  if (normalized !== relativePath || normalized === ".." || normalized.startsWith(`..${path18.sep}`)) {
    throw new Error(`invalid implementer lease resource path: ${relativePath}`);
  }
  return normalized;
}
function resourcePath(leaseDirectory, relativePath) {
  return path18.join(leaseDirectory, normalizeRelativePath(relativePath));
}
...
function recordImplementerLeaseResourceTree(record, leaseDirectory, relativeRoot) {
  ...
  const visit = (candidate, relativePath) => {
    ...
    for (const name of readdirSync2(candidate).sort()) {
      visit(path18.join(candidate, name), path18.join(relativePath, name));
    }
  };
...
function implementerLeaseDirectory(commonGitDir, token2) {
  return path19.join(implementerRegistryPaths(commonGitDir).leases, token2);
}
...
function createImplementerLeaseRecord(record, onStaged) {
  ...
  try {
    const recordPath = path19.join(finalDirectory, "lease.json");
...
function removeImplementerLeaseRecord(commonGitDir, token2) {
  ...
  const retired = path19.join(paths.retired, `.lease-${token2}-${process.pid}-${randomBytes7(4).toString("hex")}.done`);
...
function workspaceLocation(record) {
  ...
  if (!path20.isAbsolute(container) || !path20.isAbsolute(worktree) || path20.basename(container) !== `ypi_ws_${record.token}` || worktree !== path20.join(container, "checkout")) {
...
function captureWorkspaceContainerIdentity(record) {
  ...
  const owner = readOwnerMarker(path20.join(container, "owner"));
...
function captureWorkspaceTreeIdentity(record) {
  ...
  const gitFile = readWorktreeGitFile(path20.join(current.worktree, ".git"));
...
function verifyWorkspaceContainer(record, worktreeState) {
  ...
  const owner = readOwnerMarker(path20.join(location.container, "owner"));
...
`))) {
  ...
  if (worktree) {
    ...
    if (worktreeState !== "capture") {
      ...
      let gitFile;
      try {
        gitFile = readWorktreeGitFile(path20.join(location.worktree, ".git"));
      } catch {
...
function retireEmptyWorkspaceContainer(record, options = {}) {
  ...
  if (entries.length === 1 && entries[0] === "owner") {
    ...
    const ownerPath = path20.join(container, "owner");
    retiredOwnerPath = path20.join(container, `.owner-retired-${randomBytes8(16).toString("hex")}`);
    ...
    retiredOwnerPath = path20.join(container, retiredNames[0]);
  } else {
...
`))) {
  ...
  if (remaining.length !== 1 || remaining[0] !== path20.basename(retiredOwnerPath)) {
...
function normalizeResourcePath(relativePath) {
  if (relativePath === "" || relativePath === "." || path21.isAbsolute(relativePath)) {
    throw new Error("implementer confinement resource path is invalid");
  }
  const normalized = path21.normalize(relativePath);
  if (normalized !== relativePath || normalized === ".." || normalized.startsWith(`..${path21.sep}`)) {
    throw new Error("implementer confinement resource path is invalid");
  }
  return normalized;
}
function parseImplementerConfinementManifest(value) {
  ...
  if (candidate.schemaVersion !== IMPLEMENTER_CONFINEMENT_SCHEMA_VERSION || typeof candidate.gitDir !== "string" || !path21.isAbsolute(candidate.gitDir) || !candidate.resources || typeof candidate.resources !== "object" || Array.isArray(candidate.resources)) {
    throw new Error("implementer confinement manifest is malformed");
  }
  const resources = {};
  for (const [relativePath, identityValue] of Object.entries(candidate.resources)) {
    ...
    if (!REQUIRED_FILES.includes(normalized) && normalized !== BASELINE_IGNORE_ROOT && !normalized.startsWith(`${BASELINE_IGNORE_ROOT}${path21.sep}`)) {
...
function verifyImplementerConfinement(manifestFile, manifestIdentity) {
  if (path21.basename(manifestFile) !== "confinement.json") {
    throw new Error("implementer confinement manifest path is invalid");
  }
  const leaseDirectory = path21.dirname(manifestFile);
  ...
  for (const required of REQUIRED_FILES) {
    assertPrivatePathIdentity(path21.join(leaseDirectory, required), manifest.resources[required]);
  }
  const baselineIgnoreRoot = path21.join(leaseDirectory, BASELINE_IGNORE_ROOT);
  const baselineEntries = new Map;
  for (const [relativePath, identity2] of Object.entries(manifest.resources)) {
    if (!relativePath.startsWith(`${BASELINE_IGNORE_ROOT}${path21.sep}`))
      continue;
  ...
  assertPrivatePathIdentity(leaseDirectory, manifest.leaseDirectoryIdentity);
  return {
    auditFile: path21.join(leaseDirectory, "writes"),
    auditIdentity: manifest.resources.writes,
    baselineIgnoreRoot,
    gitDir: manifest.gitDir,
    scopeFile: path21.join(leaseDirectory, "scope"),
    scopeIdentity: manifest.resources.scope,
    submodulePathsFile: path21.join(leaseDirectory, "submodules"),
...
function assertWorktreeUnregistered(inventory, worktree) {
  if (parseWorktreeInventory(inventory).includes(path22.resolve(worktree))) {
...
function parseGitlinks(indexEntries, root) {
  ...
  for (const record of indexEntries.split("\x00")) {
    ...
    const submoduleRoot = path23.join(root, ...gitPath2.split("/"));
    links.push({
      path: gitPath2,
      oid,
      initialized: existsSync7(path23.join(submoduleRoot, ".git"))
    });
  }
  return links;
}
function materializeBaselineIgnoreFiles(input, root, baselineHead, ignoreRoot) {
  ...
  for (const record of listing.split("\x00")) {
    ...
    if (gitPath2.split("/").at(-1) !== ".gitignore")
      continue;
    const destination = path23.join(ignoreRoot, ...gitPath2.split("/"));
    ensurePrivateDescendantDirectory(ignoreRoot, path23.dirname(destination));
...
function createConfinementState(input, root, baselineHead, leaseDirectory, scope, record) {
  const auditFile = path23.join(leaseDirectory, "writes");
  const baselineIgnoreRoot = path23.join(leaseDirectory, "baseline-ignore");
  const childPidFile = path23.join(leaseDirectory, "child-pid");
  const childReadyFile = path23.join(leaseDirectory, "child-ready");
  const confinementFile = path23.join(leaseDirectory, "confinement.json");
  const scopeFile = path23.join(leaseDirectory, "scope");
  const submodulePathsFile = path23.join(leaseDirectory, "submodules");
...
function auditedPaths(confinement, root) {
  ...
  for (const relativePath of paths) {
    ...
    const absolutePath = path23.resolve(root, ...relativePath.split("/"));
...
function bestEffortDiscardSetupWorktree(root, record) {
  if (record.worktreeContainer && record.worktreeRoot) {
    if (!path23.isAbsolute(record.worktreeContainer) || !path23.isAbsolute(record.worktreeRoot) || path23.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path23.join(record.worktreeContainer, "checkout")) {
  ...
  if (record.worktreeContainer && existsSync7(record.worktreeContainer)) {
    ...
    try {
      ...
      const markerPath = path23.join(record.worktreeContainer, "owner");
      const marker = lstatSync8(markerPath);
      if (!container.isDirectory() || container.isSymbolicLink() || path23.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path23.join(record.worktreeContainer, "checkout") || !marker.isFile() || marker.isSymbolicLink() || readFileSync7(markerPath, "utf8").trim() !== record.token) {
...
function createGitWorktreeLease(input, root, scope) {
  ...
  try {
    assertAdmissibleCheckout(input, root);
    record.worktreeContainer = path23.join(tmpdir5(), `ypi_ws_${record.token}`);
    record.worktreeRoot = path23.join(record.worktreeContainer, "checkout");
    ...
    atomicCreateFile(path23.join(record.worktreeContainer, "owner"), `${record.token}
`);
...
function createContextFile(input) {
  if (input.context !== undefined) {
    ...
    const contextPath = path24.join(owner.path, "context.txt");
...
function createPromptFile(prompt) {
  const owner = createOwnedPrivateTempDirectory(path24.join(tmpdir6(), "ypi_prompt_"));
  const promptPath = path24.join(owner.path, "prompt.txt");
...
function createStandaloneSystemPrompt(input, promptFile, contextFile) {
  if (!input.systemPromptPath || !existsSync8(input.systemPromptPath))
    return;
  const owner = createOwnedPrivateTempDirectory(path24.join(tmpdir6(), "ypi_system_"));
  const outputPath = path24.join(owner.path, "system-prompt.md");
...
function configuredParentAgentDir() {
  ...
  if (!configured)
    return path24.join(home, ".pi", "agent");
  if (configured === "~")
    return home;
  if (configured.startsWith("~/") || configured.startsWith("~\\")) {
    return path24.join(home, configured.slice(2));
  }
  return configured;
}
...
function acquireChildResources(input) {
  ...
  try {
    ...
    if (input.fullResourceIsolation) {
      const isolatedPiOwner = createOwnedPrivateTempDirectory(path24.join(tmpdir6(), "ypi_isolated_pi_"));
...
Inherited concurrency-slot resume also failed: ${resumeFailure.message}` : primaryMessage;
  ...
  try {
    ...
    if (resources.isolatedPiRoot) {
      env.PI_CODING_AGENT_DIR = path25.join(resources.isolatedPiRoot, "agent");
      env.PI_OFFLINE = "1";
    }
    ...
    const processResult = await runChildProcess({
      ...
      launchGate: {
        launcherPath: path25.join(runtime.root, "scripts", "launch-recursive-child.ts"),
    ...
    const usageAttribution = {
      ...
      ...resources.childSession ? { session_file: path25.basename(resources.childSession) } : {},
      provider,
      model,
    ...
    const details = {
      ...
      callCount,
      treeGeneration,
      sessionFile: resources.childSession ? path25.basename(resources.childSession) : undefined,
...
var moduleExtensionPath = path26.basename(modulePath) === "rlm_query.mjs" && path26.basename(path26.dirname(modulePath)) === "dist" ? path26.join(path26.dirname(modulePath), "..", "extensions", "recursive.ts") : fileURLToPath2(new URL("../recursive.ts", import.meta.url));
    ...
    if (flags.async) {
      ...
      try {
        ...
        await writeStdout(`${JSON.stringify({
          job_id: path26.basename(path26.dirname(job.jobPath)),
...
`);
        ...
        if (job && controller.signal.aborted) {
          ...
          if (terminal) {
            ...
            console.error(`Async job did not publish terminal state; preserved at ${path26.dirname(job.jobPath)}`);
          }
...
var invokedPath = process.argv[1] ? path26.resolve(process.argv[1]) : "";
