...
import { accessSync, constants as constants4, existsSync as existsSync4, mkdirSync as mkdirSync2 } from "node:fs";
...
function hardenActiveRootSessionFile(candidate) {
  ...
  if (!path4.isAbsolute(candidate) || path4.normalize(candidate) !== candidate) {
    throw controlError3("Root session path must be absolute and normalized");
  }
  if (realpathSync.native(candidate) !== candidate) {
    throw controlError3("Root session path must not contain symlinks");
  }
  const parent = path4.dirname(candidate);
  const parentMetadata = lstatSync3(parent, { bigint: true });
  if (!parentMetadata.isDirectory() || parentMetadata.isSymbolicLink()) {
    throw controlError3(`Root session parent is not a regular directory: ${parent}`);
  }
  assertOwner(parentMetadata, parent);
  const namedBefore = lstatSync3(candidate, { bigint: true });
  if (!namedBefore.isFile() || namedBefore.isSymbolicLink()) {
    throw controlError3(`Root session path is not a regular file: ${candidate}`);
  }
  assertOwner(namedBefore, candidate);
  if (namedBefore.nlink !== 1n) {
    throw controlError3(`Root session file must be singly linked: ${candidate}`);
  }
  ...
  if (previousRaw) {
    ...
    if (previous.device !== namedBefore.dev.toString() || previous.inode !== namedBefore.ino.toString()) {
      throw controlError3(`Root session file identity changed: ${candidate}`);
    }
  }
  const descriptor = openSync3(candidate, constants3.O_RDONLY | (constants3.O_NOFOLLOW || 0));
  try {
    const opened = fstatSync3(descriptor, { bigint: true });
    if (!opened.isFile() || !sameInode(namedBefore, opened)) {
      throw controlError3(`Root session identity changed before hardening: ${candidate}`);
    }
    ...
    const namedAfter = lstatSync3(candidate, { bigint: true });
    if (!sameInode(opened, hardened) || !sameInode(opened, namedAfter) || hardened.nlink !== 1n || process.platform !== "win32" && Number(hardened.mode & 0o777n) !== PRIVATE_FILE_MODE) {
      throw controlError3(`Root session identity changed during hardening: ${candidate}`);
    }
...
function normalizedPath(filePath) {
  ...
  return existsSync3(resolved) ? realpathSync2(resolved) : resolved;
}
...
function shellHelperEnabled(runtime) {
  return process.env.YPI_SHELL_HELPER === "1" && existsSync4(runtime.rlmQueryPath);
}
...
function ensureRuntimeStatePaths() {
  ...
  const stateRoot = createPrivateTempDirectory(path6.join(tmpdir(), `ypi_runtime_${stateLabel}_`));
...
import { existsSync as existsSync5, readFileSync as readFileSync4, rmSync } from "node:fs";
...
import { closeSync as closeSync4, existsSync as existsSync6, fchmodSync as fchmodSync4, openSync as openSync4, writeSync as writeSync2 } from "node:fs";
...
function openSecureDirectory(directoryPath) {
  ...
  let resolved;
  try {
    resolved = realpathSync3.native(normalized);
  } catch {
...
function parseGitlinks(indexEntries, root) {
  ...
  for (const record of indexEntries.split("\x00")) {
    ...
    links.push({
      path: gitPath2,
      oid,
      initialized: existsSync8(path23.join(submoduleRoot, ".git"))
    });
  }
  return links;
}
...
function assertSubmodulesUnchanged(root, gitlinks) {
  for (const entry of gitlinks) {
    ...
    if (!entry.initialized) {
      if (existsSync8(path23.join(submoduleRoot, ".git"))) {
        throw new Error(`Previously uninitialized submodule was initialized during implementation: ${entry.path}`);
      }
      if (existsSync8(submoduleRoot) && readdirSync5(submoduleRoot).length > 0) {
...
function initializeChildSession(input, childSession) {
  ...
  if (input.fork && parentSession && existsSync9(parentSession)) {
