...
import { existsSync as existsSync5, readdirSync as readdirSync2, rmSync as rmSync4, statSync } from "node:fs";
...
function createOwnedContext(context) {
  const contextDir = mkdtempSync(path3.join(process.env.TMPDIR || tmpdir2(), "ypi_async_context_"));
  const contextPath = path3.join(contextDir, "context.txt");
  writeFileSync(contextPath, context, { mode: 384 });
  return contextPath;
}
function createAsyncJob(input) {
  ...
  const jobPath = path3.join(root, `${id}.job.json`);
  return {
    prompt: input.prompt,
    fork: input.fork,
    ...
    ownedContextPath,
    outputPath: path3.join(root, `${id}.txt`),
    sentinelPath: path3.join(root, `${id}.done`),
    jobPath,
  ...
  try {
    for (const name of readdirSync("/tmp")) {
      if (!name.startsWith("pi_peer_"))
        continue;
      const directory = path3.join("/tmp", name);
      try {
        const metadata = JSON.parse(readFileSync(path3.join(directory, "meta.json"), "utf8"));
        if (Number(metadata.pid) === pid)
          return path3.join(directory, "inbox.jsonl");
      } catch {}
    }
  } catch {}
  return;
...
function notifyPeer(job) {
  ...
  let result = "";
  try {
    result = readFileSync(job.outputPath, "utf8").slice(-50000);
  } catch {}
  const message = {
...
${result}`,
    ...
    id: `async_${path3.basename(job.outputPath, ".txt")}`
  };
...
`);
    ...
    if (job.ownedContextPath)
      rmSync(path3.dirname(job.ownedContextPath), { recursive: true, force: true });
  }
}
...
import { constants, existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync2, rmSync as rmSync2, writeFileSync as writeFileSync2 } from "node:fs";
...
async function allocateCallCount() {
  const counterFile = process.env.RLM_CALL_COUNTER_FILE || path4.join(tmpdir3(), "rlm_calls_default.counter");
  process.env.RLM_CALL_COUNTER_FILE = counterFile;
  const lockDir = `${counterFile}.lock`;
  mkdirSync2(path4.dirname(counterFile), { recursive: true });
...
function removePathEntry(currentPath, entry) {
  if (!currentPath)
    return currentPath;
  return currentPath.split(path5.delimiter).filter((candidate) => candidate && candidate !== entry).join(path5.delimiter);
}
...
function normalizeChildOutput(result, jsonMode) {
...
function runChildProcess(args, env, cwd, timeoutSeconds, signal) {
  return new Promise((resolve, reject) => {
    const child = spawn2(process.env.YPI_PI_BIN || "pi", args, {
      cwd,
      env,
...
import { copyFileSync, existsSync as existsSync3, mkdirSync as mkdirSync3, mkdtempSync as mkdtempSync2, rmSync as rmSync3, writeFileSync as writeFileSync3 } from "node:fs";
import { tmpdir as tmpdir4 } from "node:os";
import path6 from "node:path";
function createContextFile(input) {
  if (input.context !== undefined) {
    const contextPath = path6.join(mkdtempSync2(path6.join(tmpdir4(), "ypi_ctx_")), "context.txt");
...
function createPromptFile(prompt) {
  const promptPath = path6.join(mkdtempSync2(path6.join(tmpdir4(), "ypi_prompt_")), "prompt.txt");
...
function createWorkspace(cwd, depth) {
  ...
  const workspacePath = mkdtempSync2(path6.join(tmpdir4(), `ypi_ws_d${depth}_`));
  const workspaceSuffix = path6.basename(workspacePath).replace(/^ypi_ws_/, "");
  ...
  if (add.status !== 0) {
    rmSync3(workspacePath, { recursive: true, force: true });
    return { cwd, mode: "none", readOnly: process.env.RLM_UNSAFE_NO_JJ_WRITE !== "1", cleanup() {} };
  }
  return {
    cwd: workspacePath,
...
function childSessionFile(input) {
  ...
  return path6.join(sessionDir, `${safeTraceId(process.env.RLM_TRACE_ID || "ypi")}_d${input.childDepth}_c${input.callCount}.jsonl`);
}
function copyForkSession(input, childSession) {
  const parentSession = input.parentSessionFile || process.env.RLM_SESSION_FILE;
  if (input.fork && childSession && parentSession && existsSync3(parentSession)) {
    copyFileSync(parentSession, childSession);
  }
}
function removeArtifact(filePath) {
  if (filePath)
    rmSync3(path6.dirname(filePath), { recursive: true, force: true });
}
...
async function runRecursiveChild(runtime, request) {
  ...
  if (childExtensionsEnabled(childDepth) && extensionPath && existsSync4(extensionPath))
    args.push("-e", extensionPath);
  else if (existsSync4(runtime.systemPromptPath))
...
function configuredExtensionPath() {
  ...
  return configured && existsSync5(configured) ? configured : null;
}
...
async function main(args = process.argv.slice(2)) {
  ...
  if (flags.async) {
    ...
    process.stdout.write(`${JSON.stringify({
      job_id: path7.basename(job.outputPath, ".txt"),
...
var invokedPath = process.argv[1] ? path7.resolve(process.argv[1]) : "";
