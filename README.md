...
4. **Symbolic access** — `$CONTEXT` holds external data, the active human root request is captured in `$RLM_ROOT_PROMPT_FILE`, and each delegated charter lives in `$RLM_PROMPT_FILE`. Pi receives delegated text through its exact non-interactive stdin path instead of a syntax-sensitive or `ARG_MAX`-bounded argv token; the same text remains symbolically addressable in `$RLM_PROMPT_FILE`. Async jobs snapshot all three task/session inputs before acknowledgement. Agents can use exact files and line-addressed edits instead of copying bulk data through model memory.
