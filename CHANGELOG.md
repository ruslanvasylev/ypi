...
- `ypi`, `rlm_query`, and `ypi-doctor` prefer the package-local exact Pi dependency over a stale PATH binary. The tested package-local Pi pin is now 0.80.7, carrying the latest Pi OpenAI/Codex Responses tool-choice and prompt-cache fixes while retaining deterministic runtime selection. The pure `pi-recursive` package follows Pi's package contract by declaring host-provided Pi and `typebox` as `"*"` peers.
